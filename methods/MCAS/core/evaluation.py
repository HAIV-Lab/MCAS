...
def metric_ood(x1, x2, stypes = ['Bas'], verbose=True):
    ...
    results = dict()
    mtypes = ['TNR', 'AUROC', 'FPR95','DTACC', 'AUIN', 'AUOUT']
    if verbose:
        print('      ', end='')
        for mtype in mtypes:
    ...
    for stype in stypes:
        ...
        CCR1 = np.abs(np.array(tpr) - 0.95)
        FPR95 = fpr[np.argmin(CCR1)]
        results[stype]['FPR95'] = 100.*FPR95

        results[stype][mtype] = 100.*(-np.trapz(1.-fpr, tpr))
        if verbose:
            print(' {val:6.3f}'.format(val=results[stype][mtype]), end='')
            print(' {val:6.3f}'.format(val=results[stype]['FPR95']), end='')
...
def compute_auroc(pred_k, pred_u, labels):
